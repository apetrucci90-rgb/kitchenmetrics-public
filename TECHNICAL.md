...
- **Testing:** thirty-six automated suites, **12 694 assertions green** (31 July 2026), run from a single runner. The costing engine, the TPC registry, the thermal geometry and the Vault's per-client data scoping each have their own (51, 179, 269 and 15), alongside a UI smoke suite, the i18n parity check, and a suite that pins the lethality kinetics. The counts are quoted from the runner rather than from memory — an earlier version of this page said 138 for the TPC registry, which had been true and had stopped being true. Every physical constant in the app is either traced to a published source or explicitly flagged as an estimate in the internal audit log.
