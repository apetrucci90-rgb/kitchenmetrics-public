...
**4,807 assertions across fifteen suites**, all green, run after every change
(19 July 2026). The figure is regenerated by `tests/run_all.js` rather than
...
| thermal_geometry | 102 | slab/cylinder/sphere eigenvalues vs Çengel |
| smoke | 70 | app wiring; asserts the harness script list matches `index.html` |
| acrylamide | 55 | Reg. (EU) 2017/2158 frying cap |
| db_contradictions | 54 | pathogen-model flags; pins the two egg custards that must keep theirs |
| cooking_temps | 50 | per-jurisdiction core-temperature registry |
| load_factor | 45 | Choi-Okos c_p vs published water and lean-beef values |
| cost_engine | 31 | costing model + regression pins for 6 fixed bugs |
| data_integrity | 30 | no row cites a source that does not exist; bounded known defects |
